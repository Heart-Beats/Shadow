...
import com.tencent.shadow.sample.host.manager.ShadowConstants
 ...
 */
class MyMultiLoaderPluginManager(context: Context) : PluginManagerThatSupportMultiLoader(context) {

    protected val TAG: String = this.javaClass.simpleName

    private val mLogger = LoggerFactory.getLogger(PluginManagerThatUseDynamicLoader::class.java)

    /**
     * 多 Loader 的 PPS 加载插件包区分不同插件包的标识
     */
    protected var pluginName: String = "my-plugin"
    ...
    protected open fun getPluginProcessServiceName(): String {
        return "com.tencent.shadow.sample.host.MyMultiLoaderPluginProcessService"
    }
    ...
    private fun launchPluginService(
        ...
        try {
            ...
            try {
                ...
                val instance =
                    Class.forName("com.hl.shadow.dynamic.impl.ServiceConnectionIml").newInstance()
            ...
            val callSuccess =
                mPluginLoader.bindPluginService(pluginIntent, object : PluginServiceConnection {
                    override fun onServiceConnected(
                        ...
                        Log.d(
                            TAG,
                            "onServiceConnected（service 已连接）: componentName ==$componentName"
                        )
                    ...
                    override fun onServiceDisconnected(componentName: ComponentName) {
                        Log.d(
                            TAG,
                            "onServiceConnection（service 断开连接）: componentName ==$componentName"
                        )
    ...
    private fun loadPluginLoaderAndRuntime(uuid: String) {
        ...
        Log.d(TAG, "载入 Loader -------------------")
        loadPluginLoader2(uuid)   // Loader 已加载之后不会进行重新加载, 即 PluginProcessService 的 mUuid 不会被更改
        Log.d(TAG, "--------------- Loader  加载结束")
    }

    protected open fun bindPluginProcessService() {
        if (mPpsController == null) {
            ...
            try {
                mPpsController.setUuidManagerForPlugin(pluginKey, UuidManagerBinder(this))
    ...
    @Throws(RemoteException::class, FailedException::class)
    private fun loadPluginLoader2(uuid: String) {
        if (mLogger.isInfoEnabled) {
            mLogger.info("loadPluginLoader mPluginLoader:$mPluginLoader")
        }

        val ppsStatus = mPpsController.getPpsStatusForPlugin(pluginKey)
        if (!ppsStatus.loaderLoaded) {
            mPpsController.loadPluginLoaderForPlugin(pluginKey, uuid)
        }
        val iBinder = mPpsController.getPluginLoaderForPlugin(pluginKey)
        if (iBinder != null) {
